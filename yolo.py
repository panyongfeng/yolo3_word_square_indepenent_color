...
class YOLO(object):
    ...
    def generate(self, FLAGS):
        ...
        #except:
            ...
            if tf.executing_eagerly():
            #if 1:
                print("=============>yes, it is executing in eager mode")
                ...
                print("=============>No, it is not executing in eager mode")
                ...
                #image = tf.reshape(image, [*self.input_shape, 3])
            ...
            print("ERROR happend when loading weight!")
        #else:
        #    print(model.layers[-1].output_shape[-1], num_anchors, len(model.output), num_classes )
        ...
        #tf.train.write_graph(self.sess.graph_def, '.', './logs/ckpt/alphabet_try.pbtxt')  
        saver.save(self.sess,save_path = "./logs/ckpt/alphabet_try.ckpt")

        #checkpoint = tf.train.Checkpoint(model=model)
        #checkpoint.save(file_prefix="./logs/ckpt/checkpoint")

        #session = self.sess
        #output_node_names = ["y3/Reshape", "y2/Reshape"]
        ...
        #    graph_io.write_graph(graphdef_frozen, "./logs/ckpt/", "direct.pb", as_text=False)
        #builder = tf.saved_model.builder.SavedModelBuilder("./logs/savedmodel")
        #builder.add_meta_graph_and_variables(session, ['image'])
        #builder.save()
    ...
    def detect_image(self, image, draw=True) -> Image:
        if tf.executing_eagerly():
            ...
            out_boxes, out_scores, out_classes, out_colors = yolo_eval(
                output,
                self.anchors,
                len(self.class_names),
                image.shape[0:2],
            ...
            start = timer()
            out_boxes, out_scores, out_classes, out_colors = self.sess.run(
                self.yolo_model.output, feed_dict={self.input: image_data})
            end = timer()

        print('Found {} boxes for {}'.format(len(out_boxes), 'img'))
        #print(np.shape(out_scores), np.shape(out_classes), np.shape(out_colors))
        if draw:
            ...
            for i, c in reversed(list(enumerate(out_classes))):
                ...
                if color > 0.5:
                    ...
                    color_s = "b"
                color_i = (int)(color*10)

                label = '{}-{} {:.2f}'.format(color_s,predicted_class, score)

                label_size = draw.textsize(label, font)

                top, left, bottom, right = box
                #print(label, np.shape(color), (left, top), (right, bottom))
