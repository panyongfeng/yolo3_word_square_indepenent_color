...
def mobilenetv2_yolo_body(inputs, num_anchors, num_classes, alpha=1.0):
    ...
    x = mobilenetv2.get_layer('block_5_project_BN').output
    x, y3 = make_last_layers_mobilenet(x, 25, 128,
                                       num_anchors * (num_classes + 6))
    ...
    y3 = tf.keras.layers.Lambda(lambda y: tf.reshape(y, [
        -1, tf.shape(y)[1],
        tf.shape(y)[2], num_anchors, num_classes + 6
    ]),
                                name='y3')(y3)


    # they are same, tf auto remove []
    #return tf.keras.models.Model(inputs, [y3])
    return tf.keras.models.Model(inputs, y3)
...
def yolo_head(feats: tf.Tensor,
    ...
    box_class_probs = tf.sigmoid(feats[..., 6:])
    box_color_probs = tf.sigmoid(feats[..., 5:6])
    return box_xy, box_wh, box_confidence, box_color_probs, box_class_probs
...
def yolo_boxes_and_scores(feats: tf.Tensor, anchors: List[Tuple[float, float]],
    ...
    box_xy, box_wh, box_confidence, box_color_probs, box_class_probs = yolo_head(
    ...
    box_colors = box_confidence * box_color_probs
    box_colors = tf.reshape(box_colors, [-1, 1])
    return boxes, box_scores, box_colors
...
def yolo_eval(yolo_outputs: List[tf.Tensor],
    ...
    # because [y3] is regarded as y3, so:
    #num_layers = len(yolo_outputs)
    num_layers = 1
    anchor_mask = [[0, 1, 2]]

    input_shape = tf.shape(yolo_outputs)[1:3] * 8
    boxes = []
    box_scores = []
    box_colors = []
    for l in range(num_layers):
      # for the same reason yolo_outputs is only 1, tf remove the array infor, so:
      #  _boxes, _box_scores = yolo_boxes_and_scores(yolo_outputs[l],
        _boxes, _box_scores, _box_color_probs = yolo_boxes_and_scores(yolo_outputs,
                                                    anchors[anchor_mask[l]],
                                                    num_classes, input_shape,
                                                    image_shape)
        boxes.append(_boxes)
        box_scores.append(_box_scores)
        box_colors.append(_box_color_probs)
    boxes = tf.concat(boxes, axis=0)
    box_scores = tf.concat(box_scores, axis=0)
    box_colors = tf.concat(box_colors, axis=0)
...
if tf.version.VERSION.startswith('1.'):
    ...
    def YoloLoss(y_true,
        ...
        class_loss = object_mask * tf.nn.sigmoid_cross_entropy_with_logits(
            labels=true_class_probs, logits=yolo_output[..., 6:])
        color_loss = object_mask * tf.nn.sigmoid_cross_entropy_with_logits(
            labels=color_probs, logits=yolo_output[..., 5:6])
        confidence_loss = tf.reduce_sum(confidence_loss) / mf
        class_loss = tf.reduce_sum(class_loss) / mf
        color_loss = tf.reduce_sum(color_loss) / mf
        ...
        if box_loss == BOX_LOSS.GIOU:
            ...
            loss += giou_loss + confidence_loss + class_loss + color_loss
            if print_loss:
                tf.print(str(idx)+':',giou_loss, confidence_loss, class_loss, color_loss, tf.reduce_sum(ignore_mask))
