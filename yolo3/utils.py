...
def get_random_data(image,
    ...
    labels_color = tf.expand_dims(labels_color, 0)
    if train:
    ...
    bbox = tf.concat([xmins, ymins, xmaxs, ymaxs,
                      tf.cast(labels, tf.float32), tf.cast(labels_color, tf.float32)], 0)
...
def preprocess_true_boxes(true_boxes, input_shape, anchors, num_classes):
    ...
    grid_shapes = [
        np.round(input_shape / [8][l]).astype(np.int32)
        for l in range(num_layers)
    ]
    y_true = [
        np.zeros((grid_shapes[l][0], grid_shapes[l][1], len(
            anchor_mask[l]), 6 + num_classes),
    ...
    for t, n in enumerate(best_anchor):
        for l in range(num_layers):
            if n in anchor_mask[l]:
                ...
                color = true_boxes[t, 5].astype('float32')
                
                y_true[l][j, i, k, 0:4] = true_boxes[t, 0:4]
                y_true[l][j, i, k, 4] = 1.
                y_true[l][j, i, k, 5] = color
